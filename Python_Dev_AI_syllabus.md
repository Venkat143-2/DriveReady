...
## Complete 24-Week Syllabus — System Design & AI Focused

> **A placement-focused, project-driven program.** Over 24 weeks across two phases, you first build strong Python engineering and system-design foundations, then specialize in AI engineering and ship a real portfolio. The heavy emphasis on Low-Level Design (LLD) and High-Level Design (HLD) is deliberate: for students, system-design skill is often the difference between a ₹6 LPA service-company offer and a ₹15–25 LPA product-company offer. Same DSA, same projects — deeper design ability.
...
> **Weekly rhythm:** a 6-day week = **4 domain days (in this doc)** + **2 DSA days (separate self-paced track)**. DSA is paced separately and not detailed here.
...
| SQL depth                         | **Full 12-topic relational module** (CRUD → joins → indexing → ACID → window functions → schema design)                                            |
...
| Assessments (contests)            | **3** (Python/OOP/Concurrency, Design Patterns, Machine Coding)                                                                                    |
...
> Covers the full OOP toolkit — abstraction, classes, encapsulation, inheritance, polymorphism, abstract classes, and interfaces — all in Python. Interfaces are taught the Python way = `abc.ABC` + `typing.Protocol` (structural typing; Python has no `interface` keyword).
...
> Covers the advanced Python toolkit plus concurrency. Key Python-specific teaching point: the **GIL** — threads give concurrency but not CPU-bound parallelism; use `multiprocessing` for that.

| Day | Topic                          | 90 min Learn                                                                                                              | 90 min Practice                                                                                 |
| --- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| 1   | Decorators & functools         | Function decorators, decorators with args, `functools.wraps`, `lambda`                                                   | Write `@timer`, `@retry`, `@cache` decorators                                                   |
| 2   | Generators, Iterators & Typing | `yield`, generators, `itertools`, context managers, type hints, generics (`TypeVar`, `Generic`), exceptions & custom errors | Build a Timer context manager + file line-reader generator; add type hints to Week 1 code       |
| 3   | Concurrency                    | Processes vs threads, `threading`, `concurrent.futures` (Executors & Callables), the **GIL caveat**, `multiprocessing` for CPU-bound work | Build a thread pool that fetches 10 URLs concurrently; measure vs sequential                    |
| 4   | Synchronization + asyncio      | Locks, `Semaphore`, race conditions → `asyncio`, event loop, `asyncio.gather`, httpx                                     | Fix a race condition with a lock/semaphore; build an async function calling 3 APIs concurrently |

> **Assessment:** Contest 1 — Python, OOP & Concurrency.

---

## Week 3 — Databases & SQL

> A full relational SQL module, taught on **PostgreSQL** and threaded to a single running example (QuickBite). Networking essentials (HTTP/REST/status codes/WebSockets/TLS) are taught in Week 5, in the context of building an API. NoSQL is introduced later in the HLD weeks (W10, W20), where its scaling trade-offs make the most sense.

| Day | Topic                              | 90 min Learn                                                                                                           | 90 min Practice                                                                        |
| --- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 1   | DB Foundations + CRUD              | Intro to databases, relational fundamentals (tables, keys, relationships), SELECT/WHERE/ORDER BY, INSERT/UPDATE/DELETE | 15 CRUD queries on a QuickBite dataset (users / orders / restaurants)                  |
| 2   | Joins + Aggregation                | All join types (inner/left/right/full/self), GROUP BY, HAVING, aggregate functions                                     | 10 multi-table join + aggregation queries; joins & aggregation practice set            |
| 3   | Subqueries, Indexing & ACID        | Subqueries & views, indexing & query optimization (when/how, EXPLAIN), transactions & ACID                             | Add indexes to a slow query and measure speedup; write a transaction with rollback     |
| 4   | Window Functions + Schema Design   | Window functions (ROW_NUMBER, RANK, running totals), Database Schema Design I & II (normalization, ER modeling)        | Write 5 window-function queries; design a normalized schema for QuickBite from scratch |
...
> Networking essentials are folded into Day 1, where HTTP methods, status codes, and REST principles are taught in the natural context of building routes.

| Day | Topic                           | 90 min Learn                                                                                                                                    | 90 min Practice                                                                                   |
| --- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 1   | Networking + FastAPI + Pydantic | HTTP methods, status codes, REST principles, WebSockets vs HTTP, TLS → FastAPI setup, routing, Pydantic models, request/response validation, auto-docs | Hit 5 public APIs and document responses; build basic CRUD API with in-memory store (5 endpoints) |
...
> **Placement-critical week #1.** Most freshers know SOLID by name but can't apply it. This week makes you actually apply it — with UML, the machine-coding approach method, and a full first design problem.
...
| 3   | UML + Extensible/AI Design         | Class diagrams (attributes, methods, relationships), sequence diagrams; designing extensible systems + AI-integration seams | Draw UML class diagrams for Week 4 + Week 7 projects                          |
| 4   | Machine Coding: Approach + Library | How to approach design problems (requirements → entities → classes → interactions → code)                                   | Design & code a **Library Management System** in Python end-to-end (with UML) |

> **Assessment:** Contest 2 — Design Principles (runs with Week 9).

---

## 🎯 Week 9 — LLD Patterns & Problems

> **Placement-critical week #2.** Solve classic LLD problems used in product company interviews. Design patterns are taught **the Python way** — Singleton via module/metaclass/decorator (not Java's private constructor), Builder often via `@dataclass`, Prototype via `copy.deepcopy`, and the Decorator pattern contrasted against Python's *native* decorators.
...
| 3   | LLD Problem Pair 1               | Approach method applied: requirements → entities → classes → interactions → code                          | Solve: **Parking Lot** + **Tic-Tac-Toe** in Python (with UML diagrams)                      |
...
> **Assessment:** Contest 3 — Machine Coding.

---

## 🎯 Week 10 — HLD Building Blocks

> **Placement-critical week #3.** Without these building blocks, every HLD interview is just guessing. **NoSQL is introduced here.**
...
> **Placement-critical week #4.** Solve real interview cases. By end of week, students can attempt any standard HLD question.
...
> **Pattern:** 4 tech days + 1 dedicated Project Build day per week (plus the separate self-paced DSA track).
...
> **Placement-critical week #5.** Goes beyond basic patterns into the harder LLD problems that separate good candidates from great ones. **Chess** lives here as a top-tier problem.
...
> **Placement-critical week #6.** Real interview HLD questions, with the advanced concepts that come up in product company interviews.
...
## 🎯 Week 21 — HLD for AI Systems

> **Placement-critical week #7.** AI companies now ask "design ChatGPT" and "design a RAG system" in interviews. This module gives you the answer.
...
> **Project: AI Interview Coach** — a multi-agent system that conducts mock interviews, evaluates answers, and provides coaching.
...
**Portfolio (6 total, 3 new this phase):**
...
## Why This Curriculum Wins Placements

This curriculum is built around system-design depth, backed by interview-grade SQL and Python-native LLD. Here's why that matters:

|                               | Typical Course | This Course                  |
| ----------------------------- | -------------- | ---------------------------- |
...
You'll be ready for the third tier — not just AI roles, but any product company AI/backend role.
...
*End of 24-week syllabus. DSA and other placement preparation are handled separately on a self-paced track.*
